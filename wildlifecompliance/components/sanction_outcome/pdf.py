...
def _create_invoice(invoice_buffer, sanction_outcome):
    ...
    doc.sanction_outcome = sanction_outcome
    # owner = sanction_outcome.owner
    ...
    # items = sanction_outcome.order.lines.all()
    # discounts = sanction_outcome.order.basket_discounts
    # if sanction_outcome.text:
    #     elements.append(Paragraph(sanction_outcome.text, styles['Left']))
    ...
    elements.append(Spacer(1, SECTION_BUFFER_HEIGHT * 2))
    # /Products Table
    if sanction_outcome.payment_status != 'paid' and sanction_outcome.payment_status != 'over_paid':
